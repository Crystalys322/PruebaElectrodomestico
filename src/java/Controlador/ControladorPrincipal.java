...
public class ControladorPrincipal extends HttpServlet {
    ...
    private void mostrarPanel(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        List<Producto> productos = productoDAO.listar();
        ...
        if (idSeleccionado != null) {
            try {
                ...
                seleccionado = productoDAO.obtenerPorId(id);
            } catch (NumberFormatException ex) {
                seleccionado = null;
            }
        }
        if (seleccionado == null && !productos.isEmpty()) {
            seleccionado = productos.get(0);
        }
        request.setAttribute("productos", productos);
        request.setAttribute("productoSeleccionado", seleccionado);
